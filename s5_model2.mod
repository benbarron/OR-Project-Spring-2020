...
## ------------------  OBJECTIVE  ---------------------------------------------------------------------
# representatios for indexes of x
# 0 hubs => x[i, i, i, j]	: i <> j
# 0 hubs => x[i, i, j, j]	: i <> j
# 0 hubs => [i, j, j, j]	: i <> j
# 1 hub with origin & destination as non hubs => x[i, h, h, j] : i <> h <> j
# 1 hub with destination as hub => x[i, h, j, j] : i <> h <> j
# 1 hub with orgin as hub => x[i, i, h, j] : i <> h <> j
# 2 hubs => x[i,h,k,j] i <> h <> k <> j
...
subject to c_hub_capacity_1{h in 1..N}:
	sum{i in 1..N, k in 1..N, j in 1..N} (x[i, h, k, j] + x[i, k, h, j]) <= sum{s in 1..S} (y[h, s] * hub_capacity[s]);

subject to c_hub_per_city{i in 1..N}:  												
	sum{s in 1..S} y[i, s] <= 1;
